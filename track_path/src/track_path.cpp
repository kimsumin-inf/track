...
:nh(""), pnh(""), loop_rate(10)
{
    ...
    sub_VEL = nh.subscribe("/bestvel",1,&track_path::vel_CB,this);
    sub_Pose = nh.subscribe("/track/track_localizer/LocalPose", 1, &track_path::pose_CB, this);
...
void track_path::pose_CB(const geometry_msgs::PoseWithCovariance::ConstPtr &msg) {
    POSE = *msg;
    if (real_speed > 3) {
        now.x = POSE.pose.position.x;
        now.y = POSE.pose.position.y;
...
void track_path::vel_CB(const novatel_gps_msgs::NovatelVelocity::ConstPtr &msg) {
    VEL = *msg;
    real_speed = sqrt(pow(VEL.horizontal_speed,2)+ pow(VEL.vertical_speed,2))*3.6;
}
...
void track_path::now_state(double distance) {
    ...
    if(distance <= interval && rising==true){
        ...
        if(map_generate_param == false){
            ...
            vec_utm.at(vec_utm.size()-2).x = (vec_utm.at(vec_utm.size()-1).x + vec_utm.at(vec_utm.size()-2).x + vec_utm.at(vec_utm.size()-3).x )/3;
            vec_utm.at(vec_utm.size()-2).y = (vec_utm.at(vec_utm.size()-1).y + vec_utm.at(vec_utm.size()-2).y + vec_utm.at(vec_utm.size()-3).y )/3;

            vec_utm.at(vec_utm.size()-3).x = (vec_utm.at(vec_utm.size()-2).x + vec_utm.at(vec_utm.size()-3).x + vec_utm.at(vec_utm.size()-4).x )/3;
            vec_utm.at(vec_utm.size()-3).y = (vec_utm.at(vec_utm.size()-2).y + vec_utm.at(vec_utm.size()-3).y + vec_utm.at(vec_utm.size()-4).y )/3;
